...
**Status:** Ingest pass 1 (5 papers) + deep-read pass (5 of 5 — Lin 2025 done 2026-05-06 via pypdf workaround) + ingest pass 2 (4-paper security cluster) + ingest pass 3 (4-paper print-farm/production-economics cluster) + ingest pass 4 (Bambu materials baseline, vendor-doc external) done 2026-05-06; ingest pass 5 (3-paper VLM-in-manufacturing cluster) + ingest pass 6 (25-repo Bambu toolchain audit) done 2026-05-07. 42 PDFs/docx remaining. Wiki has **41 pages** (18 source + 15 concept + 5 materials entity + 2 slicer entity + 1 tools entity).

Source-mix surprise (still applies): ~58 of original 62 are academic AM/3D-printing papers. Curation surfaces practical takeaways for the friend; some papers may go in as background-context one-liners rather than full pages.

**Cluster picked first:** print-quality control on consumer-grade FDM (input shaping, extrusion control, fault detection, high-speed FDM). Reasoning — directly relevant to a friend buying a Bambu, and the 5 papers cross-reference cleanly into a connected cluster.

**Scope still to populate (subsequent passes):**
- Printers: Bambu X1C (or whichever model the friend chooses), comparable printers for context
...
- Security: side-channel IP-theft attacks (3+ papers in inbox — relevant if friend wants to protect commercial designs)
- Volumetric AM, neuromorphic anomaly detection, photonics (likely background-only; friend won't be running these)

---

## Open decisions

- Friend's specific Bambu model (X1C? P1S? A1?) — defer until friend chooses; entity pages can stay generic-Bambu until then
...
|------|------|-----------------|
| 2026-05-06 | Workspace scaffolded (HEAVY mode) | Friend's primary research hub for Bambu Labs printer purchase + design + store ops; populated before handoff |
...
- Bambu-specific entity pages (X1C / P1S / A1 / A1 mini) once friend chooses model — completes the printer-vs-material compatibility hop that materials cluster currently leaves at the table-row level.
...
- Friend handoff: README + first-run walkthrough + `.env` setup walk-through (do this only after ingest is well underway and core entity/concept pages exist)
